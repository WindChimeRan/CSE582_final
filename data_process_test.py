...
def group_words(entry, tables):
    ...
    while idx < num_toks:
        ...
        if res:
            ...
            entry[Keys.META].append(
                build_entry(this_entry, tokens, cls, sim_cls, idx, endIdx)
            )
            idx = endIdx
            continue
...
def parse_args():
    ...
    parser.add_argument("--tok", dest="tok", default=None, type=str)
    parser.add_argument("--table", dest="table", default=None, type=str)
    parser.add_argument("--out", dest="out", default=None, type=str)
    parser.add_argument("--data_dir", dest="data_dir", default="data", type=str)
    parser.add_argument("--out_dir", dest="out_dir", default="tmp", type=str)
    if len(sys.argv) == 1:
        parser.print_help()
        sys.exit(1)
