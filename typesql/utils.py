...
def best_model_name(args, for_load=False):
    ...
    agg_model_name = args.sd + "/%s_%s%s.agg_model" % (new_data, mode, use_emb)
    sel_model_name = args.sd + "/%s_%s%s.sel_model" % (new_data, mode, use_emb)
    cond_model_name = args.sd + "/%s_%s%s.cond_model" % (new_data, mode, use_emb)
...
def to_batch_seq(sql_data, table_data, idxes, st, ed, db_content=0, ret_vis_data=False):
    ...
    for i in range(st, ed):
        ...
        )  # op num rep in condition, then where is str in cond?
...
def epoch_train(
    model, optimizer, batch_size, sql_data, table_data, pred_entry, db_content
):
    model.train()
...
def load_concat_wemb(fn1, fn2):
    ...
    comb_emb = {
        k: np.concatenate((wemb1.get(k, backup), wemb2.get(k, backup)), axis=0)
...
def load_word_and_type_emb(
    fn1, fn2, sql_data, table_data, db_content, is_list=False, use_htype=False
):
