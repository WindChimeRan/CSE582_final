...
class SQLNet(nn.Module):
    def __init__(
        ...
        self.agg_type_embed_layer = WordEmbedding(
            word_emb, N_word, gpu, self.SQL_TOK, trainable=is_train
        )
        self.sel_type_embed_layer = WordEmbedding(
            word_emb, N_word, gpu, self.SQL_TOK, trainable=is_train
        )
        self.cond_type_embed_layer = WordEmbedding(
            word_emb, N_word, gpu, self.SQL_TOK, trainable=is_train
        )

        self.embed_layer = WordEmbedding(
            word_emb, N_word, gpu, self.SQL_TOK, trainable=trainable_emb
        )
        ...
        self.op_str_pred = CondOpStrPredictor(
            N_word, N_h, N_depth, self.max_col_num, self.max_tok_num, gpu, db_content
        )
    ...
    def forward(
        ...
        if self.trainable_emb:
            ...
            if pred_agg:
                ...
                agg_score = self.agg_pred(
                    x_emb_var, x_len, agg_emb_var, col_inp_var, col_len
                )
            ...
            if pred_sel:
                ...
                sel_cond_score = self.selcond_pred(
                    x_emb_var, x_len, col_inp_var, col_len, x_type_sel_emb_var, gt_sel
                )
            ...
            if pred_cond:
                ...
                cond_op_str_score = self.op_str_pred(
                    x_emb_var,
                    x_len,
                    col_inp_var,
                    col_len,
            ...
            if pred_sel:
                sel_cond_score = self.selcond_pred(
                    x_emb_var, x_len, col_inp_var, col_len, x_type_emb_var, gt_sel
                )
        ...
        if pred_cond:
            ...
            for b in range(len(gt_where)):
                for idx in range(len(gt_where[b])):
                    ...
                    if self.gpu:
                        cond_str_truth_var = (
                            Variable(data).type(torch.LongTensor).cuda()
                        )
                    else:
    ...
    def check_acc(
        ...
        for b, (pred_qry, gt_qry, vis_data) in enumerate(
            zip(pred_queries, gt_queries, vis_info)
        ):
            good = True
            if pred_agg:
            ...
            if pred_cond:
                ...
                for idx in range(len(cond_pred)):
                    ...
                    if (
                        flag
                        and str(cond_gt[gt_idx][2]).lower()
    ...
    def predict_SQL_with_EG(self, engine, dataset, beam_size=5, model_outputs=None):
        ...
        for input_feature, model_output in zip(dataset.input_features, model_outputs):
            ...
            for condition in conditions:
                ...
                word_start, word_end = (
                    input_feature.subword_to_word[wc][vs],
                    input_feature.subword_to_word[wc][ve],
                )
