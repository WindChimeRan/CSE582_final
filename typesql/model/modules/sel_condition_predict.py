...
class SelCondPredictor(nn.Module):
    ...
    def forward(self, x_emb_var, x_len, col_inp_var, col_len, x_type_emb_var, gt_sel):
        ...
        sel_score = self.sel_out(
            self.sel_out_K(K_sel_expand) + self.sel_out_col(e_col)
        ).squeeze()
        ...
        att_num_type = self.softmax(att_num_type_val.view((-1, max_x_len))).view(
            B, -1, max_x_len
        )
        ...
        att_sel_val = torch.bmm(
            self.col_att(h_enc), chosen_e_col.unsqueeze(2)
        ).squeeze()
        ...
        cond_col_score = self.cond_col_out(
            ...
            + self.cond_col_out_sel(K_sel_agg.unsqueeze(1).expand_as(K_cond_col))
        ).squeeze()
