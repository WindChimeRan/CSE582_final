...
class AggPredictor(nn.Module):
    ...
    def forward(self, x_emb_var, x_len, agg_emb_var, col_inp_var=None, col_len=None):
        B = len(x_emb_var)
        max_x_len = max(x_len)
        ...
        agg_score = self.agg_out_f(
            self.agg_out_se(agg_emb_var) + self.agg_out_K(K_agg_expand)
        ).squeeze()
