...
class CondOpStrPredictor(nn.Module):
    ...
    def gen_gt_batch(self, split_tok_seq):
        B = len(split_tok_seq)
        max_len = (
            max([max([len(tok) for tok in tok_seq] + [0]) for tok_seq in split_tok_seq])
            - 1
    ...
    def forward(
        ...
        for b in range(B):
            cur_col_emb = torch.stack(
                [e_col[b, x] for x in chosen_col_gt[b]]
        ...
        op_att_val = torch.matmul(
            self.cond_op_att(h_enc).unsqueeze(1), col_emb.unsqueeze(3)
        ).squeeze()
