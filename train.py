...
if __name__ == "__main__":
    ...
    parser.add_argument("--sd", type=str, default="", help="set model save directory.")
    parser.add_argument(
        "--db_content",
        type=int,
        default=0,
    ...
    parser.add_argument(
        "--train_emb", action="store_true", help="Train word embedding."
    )
    ...
    (
        ...
        DEV_DB,
        TEST_DB,
    ) = load_dataset(use_small=USE_SMALL)

    # word_emb = load_word_emb('glove/glove.%dB.%dd.txt'%(B_word,N_word), \
    #        load_used=args.train_emb, use_small=USE_SMALL)
    if args.db_content == 0:
        word_emb = load_word_and_type_emb(
        ...
        word_emb = load_concat_wemb(
            "glove/glove.42B.300d.txt", "para-nmt-50m/data/paragram_sl999_czeng.txt"
        )
    ...
    for i in range(10):
        ...
        val_acc = epoch_acc(
            ...
            args.db_content,
            False,
        )  # for detailed error analysis, pass True to the end
        ...
        if TRAIN_AGG:
            ...
            torch.save(
                model.agg_type_embed_layer.state_dict(),
        ...
        if TRAIN_SEL:
            if val_acc[1][1] > best_sel_acc:
                ...
                best_sel_idx = i + 1
                torch.save(
                    model.selcond_pred.state_dict(),
                    args.sd + "/epoch%d.sel_model%s" % (i + 1, args.suffix),
                )
                torch.save(model.selcond_pred.state_dict(), sel_m)

                torch.save(
                    model.sel_type_embed_layer.state_dict(),
        ...
        if TRAIN_COND:
            if val_acc[1][2] > best_cond_acc:
                ...
                best_cond_idx = i + 1
                torch.save(
                    model.op_str_pred.state_dict(),
                    args.sd + "/epoch%d.cond_model%s" % (i + 1, args.suffix),
                )
                torch.save(model.op_str_pred.state_dict(), cond_m)

                torch.save(
                    model.cond_type_embed_layer.state_dict(),
